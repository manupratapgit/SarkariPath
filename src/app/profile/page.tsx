...
const EXAMS = ["UPSC", "SSC", "Railways (RRB)", "Banking (IBPS/SBI)", "Defence", "Teaching", "State PSC"];
const QUALIFICATIONS = ["10th Pass", "12th Pass", "Diploma", "Graduate", "Post Graduate", "PhD"];
const CATEGORIES = ["General", "OBC", "SC", "ST", "EWS", "PwD"];
const STATES = ["Andhra Pradesh","Bihar","Delhi","Gujarat","Haryana","Jharkhand","Karnataka","Kerala","Madhya Pradesh","Maharashtra","Punjab","Rajasthan","Tamil Nadu","Telangana","Uttar Pradesh","Uttarakhand","West Bengal","Other"];

const EMPTY: Profile = {
  full_name: "", phone: "", date_of_birth: "", gender: "",
  state: "", city: "", highest_qualification: "", field_of_study: "",
  category: "", preferred_exams: [],
};
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input value={profile.full_name} onChange={(e) => setProfile(p => ({ ...p, full_name: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" placeholder="Manu Pratap" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-600 mb-1">Phone</label>
                <input value={profile.phone} onChange={(e) => setProfile(p => ({ ...p, phone: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" placeholder="10-digit mobile number" />
              </div>
              <div>
              ...
              <div>
                ...
                <input value={profile.city} onChange={(e) => setProfile(p => ({ ...p, city: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" placeholder="Your city" />
              </div>
            </div>
          </section>
          ...
          <section className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input value={profile.field_of_study} onChange={(e) => setProfile(p => ({ ...p, field_of_study: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" placeholder="e.g. Engineering, Arts" />
              </div>
              <div>
